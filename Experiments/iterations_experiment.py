...
class IterationsExperiment(BaseExperiment):
    ...
    def __init__(self, original_images: ThreeDMatrix, data_type: str,
        ...
        log_fields: List[str] = [
            ...
            LogFields.RegularizarionParameter, LogFields.SinogramError, LogFields.RMSError]
    ...
    def run(self) -> DataLog:
        ...
        for snr in self._snr_list:
            ...
            for solver_name in self._solvers_list:
                min_error_per_solver[solver_name] = (None, None, -1, np.inf, None)
            ...
            for solver_name, alpha in product(self._solvers_list, self._alphas_list):
                ...
                for i in range(self._max_iterations):
                    ...
                    for j, sinogram in enumerate(noisy_sinograms):
                        if is_sart:
                            estimated_images[j], sinogram_error = solver(
                                sinogram, self._thetas, R, estimated_images[j])
                        else:
                            estimated_images[j], sinogram_error = solver(
                    ...
                    rms_error = error_in_circle_pixels(self._true_images, estimated_images)
                    ...
                    self.data_log.append_dict({
                        ...
                        LogFields.SNR: snr,
                        LogFields.RMSError: rms_error,
                        LogFields.SinogramError: sinogram_error,
                        LogFields.RegularizarionParameter: alpha_str
                    })

                    # Keeping (for plotting) the alpha and estimated image,
                    # for which the sinogram discrepancy is minimal.
                    if min_error_per_solver[solver_name][3] > sinogram_error:
                        min_error_per_solver[solver_name] = (estimated_images.copy(), alpha_str, i + 1, sinogram_error, rms_error)
    ...
    def plot(self, plot_name=None):
        ...
        groupby_second_columns = [LogFields.SolverName]
        for name, group in df.groupby(groupby_first_columns):
            ...
            for name2, inner_group in group.groupby(groupby_second_columns):
                print(name2)
                graph_label = r'\textit{' + name2 + ', } '
                best_alpha = self._calculated_output_images[name][name2][1]
                if best_alpha != 'None':
                    graph_label += r'$\alpha$ = ' + str(best_alpha)
                matching_rows = inner_group.loc[inner_group[LogFields.RegularizarionParameter] == best_alpha]
                matching_rows.plot(x=LogFields.Iterations, y=LogFields.RMSError,
            ...
            for name2, inner_group in group.groupby(groupby_second_columns):
                    ...
                    if best_alpha != 'None':
                        graph_label += r'$\alpha$ = ' + str(best_alpha)
                    matching_rows = inner_group.loc[inner_group[LogFields.RegularizarionParameter] == best_alpha]
                    matching_rows.plot(x=LogFields.Iterations, y=LogFields.SinogramError,
                                    ax=ax, label=graph_label)
                    plt.xlabel('\\textit{' + LogFields.Iterations + '}', fontsize=12)
                    plt.ylabel('\\textit{' + LogFields.SinogramError + '}', fontsize=12)
                    title = 'Solvers Sinogram Error for SNR = ' + str(name) + ", " + str(len(self._thetas)) + " projections"
                    title = '\\textit{' + title + '}'
                    plt.title(title, fontsize=16)
        ...
        for snr in self._calculated_output_images.keys():
            ...
            if first_snr:
                ...
                gs = fig.add_gridspec(2, len(self._solvers_list))

            # Each figure contains all results for all solvers. Only the best regularization parameters are shown here.
            for index, (solver_name, (estimated_images, alpha, iterations, sinogram_error, rms_error)) in \
                ...
                estimation_ax.set_xlabel(r'\textit{RMS Error ' + str(rms_error)[:6] + '}', fontsize=16)
                ...
                error_ax.set_xlabel(r'\textit{Sinogram Error ' + str(sinogram_error)[:6] + '}', fontsize=16)
